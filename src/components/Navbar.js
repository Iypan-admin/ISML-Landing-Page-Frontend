...
function Navbar({ showHeroTextInNav }) {
  ...
  const handleNavClick = () => {
    ...
    setIsMoreOpen(false); // Also close more dropdown
  };
  ...
  const handleCloseMobileMenu = (e) => {
    ...
    setIsMoreOpen(false); // Close more dropdown
  };
  ...
  const handleLanguagesToggle = (e) => {
    ...
    if (isMoreOpen) setIsMoreOpen(false); // Close more if languages opened
  };

  // Toggle More section on mobile
  const handleMoreToggle = (e) => {
    e.preventDefault();
    setIsMoreOpen(!isMoreOpen);
    if (isLanguagesOpen) setIsLanguagesOpen(false); // Close languages if more opened
  };
    ...
    <nav className={`navbar${showHeroTextInNav ? ' compact' : ''}${!isVisible ? ' navbar-hidden' : ''}`}>
      <div className="navbar-container">
        ...
        <div className="navbar-socials mobile-socials">
          <a href="https://wa.me/917338881781" target="_blank" rel="noopener noreferrer">
            <FaWhatsapp className="navbar-social-icon whatsapp-icon" />
          </a>
